...
const ReviewTabs: React.FC = () => {
  ...
  const [currentBreadReviewIndex, setCurrentBreadReviewIndex] = useAtom(
    currentBreadReviewIndexAtom
  );
...
const Tab = styled.button<{ isActive?: boolean }>`
  ...
  border-bottom: ${({ theme, isActive }) =>
    isActive ? `3px solid ${theme.color.primary500}` : `3px solid transparent`};
  outline: none;
  ...
  color: ${({ theme, isActive }) =>
    isActive ? theme.color.black : theme.color.gray700};
